...
if ($lastid == $count) {
  ...
  while ($row = $res->fetch_assoc()) {
    ...
    if ($category) {
      @$post_category = "<a href=\"$li_URL/news/$cate/\">$category</a> | ";
    } 
    else {
      @$post_category = "";
    }
    ...
    if (isset($_GET['id']) and $text2) {
      @$post_text2 = "<p>$text2</p>";
    }
    if ($text2 and !@$_GET['id']) {
      @$post_text2_link =  "<a href=\"$li_URL/news?id=$posid\">Читать далее...</a>";
    } 
    else {
      @$post_text2_link =  "";
    }
